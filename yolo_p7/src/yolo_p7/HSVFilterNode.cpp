...
#include "yolo_p7/HSVFilterNode.hpp"
...
const std::string sensitivity_slider = "Sensitivity";
...
int sensitivity = 160;  // Default sensitivity value


void create_trackbars(int h_min, int h_max, int s_min, int s_max, int v_min, int v_max)
{
  cv::namedWindow(window_name);
  cv::createTrackbar("Low H", window_name, nullptr, h_max_val);
  cv::setTrackbarPos("Low H", window_name, h_min);
  cv::createTrackbar("High H", window_name, nullptr, h_max_val);
  cv::setTrackbarPos("High H", window_name, h_max);
  cv::createTrackbar("Low S", window_name, nullptr, s_max_val);
  cv::setTrackbarPos("Low S", window_name, s_min);
  cv::createTrackbar("High S", window_name, nullptr, s_max_val);
  cv::setTrackbarPos("High S", window_name, s_max);
  cv::createTrackbar("Low V", window_name, nullptr, v_max_val);
  cv::setTrackbarPos("Low V", window_name, v_min);
  cv::createTrackbar("High V", window_name, nullptr, v_max_val);
  cv::setTrackbarPos("High V", window_name, v_max);
  cv::createTrackbar(sensitivity_slider, window_name, &sensitivity, 500);
}
...
HSVFilterNode::filter_image(cv::Mat & in, int h, int s, int v, int H, int S, int V)
{
  ...
  int num_labels = cv::connectedComponentsWithStats(out, labels, stats, centroids);
  
  // Get sensitivity from trackbar
  int min_area = cv::getTrackbarPos(sensitivity_slider, window_name);
  cv::Mat filtered = cv::Mat::zeros(out.size(), CV_8UC1);
